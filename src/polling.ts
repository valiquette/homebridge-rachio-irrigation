...
export default class poll {
	...
	async getUpdate(valveService: any, interval: any) {
		const pause = (delay: any) => new Promise(resolve => setTimeout(resolve, delay));
		const valveId = valveService.subtype;

		try {
			this.log.debug('updating valve Id', valveId);
			const response = await this.rachioapi.getValve(this.platform.token, valveId).catch((err: any) => {
